...
export default function AuthScreen({ onBack, onAuthSuccess }: AuthScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      try {
        ...
        setError("Network connection failure with Lumina nodes.");
        setIsLoading(false);
      }
    }
  };
  ...
  const handleGoogleSelect = async (selectedEmail: string) => {
    ...
    try {
      ...
      setError("Failed to coordinate Google OAuth token.");
      setIsLoading(false);
    }
  };
  ...
  const handleOAuth = (provider: string) => {
    if (provider === "Google") {
      ...
      // Simulate waiting for Google authorization and retrieve active ID
      setTimeout(() => {
        if (popup) {
          try {
            popup.close();
        ...
        handleGoogleSelect("Kkjagadale@yahoo.com");
      }, 2000);
      return;
    }
    setIsLoading(true);
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans flex flex-col lg:flex-row relative overflow-hidden" id="auth-root-container">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12 relative z-10" id="auth-right-panel">
        ...
        <motion.div 
          ...
          className="w-full max-w-[440px] glass-card p-8 rounded-3xl border border-white/5 shadow-2xl relative" 
        ...
        >
          ...
          <div className="text-center space-y-2 mb-6" id="auth-card-header">
            <div className="inline-flex w-10 h-10 rounded-xl bg-gradient-to-tr from-brand-violet to-brand-magenta items-center justify-center text-white mb-2 shadow-lg shadow-brand-violet/20">
          ...
          >
            ...
            {isSignUp && (
              <motion.div variants={itemStagger} className="flex items-start space-x-2 pt-1" id="form-group-terms">
                <input
                  ...
                  className="mt-0.5 rounded border-white/10 bg-white/5 text-brand-violet focus:ring-brand-violet focus:ring-opacity-25"
                />
            ...
            {!isSignUp && (
              <motion.div variants={itemStagger} className="text-right" id="forgot-password-link">
                <span className="text-[10px] text-gray-500 hover:text-brand-violet cursor-pointer transition-colors font-semibold">
                  Forgot Password?
                </span>
              </motion.div>
            )}
            ...
            <motion.button
              type="submit"
              disabled={isLoading}
              className="w-full py-3.5 bg-gradient-to-r from-brand-violet to-brand-magenta hover:shadow-xl hover:shadow-brand-violet/20 text-white font-extrabold text-xs rounded-xl transition-all flex items-center justify-center space-x-2 disabled:opacity-50 cursor-pointer mt-4"
            ...
            >
              ...
              <div className="space-y-4">
                <div className="text-center">
                  ...
                  <span className="block text-[10px] text-gray-400 mt-1">Select an account to authorize secure session</span>
                </div>

                {!showCustomGoogleInput ? (
                  <div className="space-y-2">
                    {/* User's Verified Email from Meta */}
                    <button 
                      onClick={() => handleGoogleSelect("Kkjagadale@yahoo.com")}
                      className="w-full p-3 rounded-xl border border-white/5 bg-white/5 hover:bg-white/[0.08] hover:border-white/15 transition-all text-left flex items-center justify-between"
                    >
                      <div>
                        <span className="block text-xs font-bold text-white">Kkjagadale@yahoo.com</span>
                        <span className="block text-[9px] text-gray-500">Kk Jagadale (Lumina Identity)</span>
                      </div>
                      <ShieldCheck className="w-4 h-4 text-brand-emerald" />
                    </button>

                    <button 
                      onClick={() => handleGoogleSelect("guest-explorer@gmail.com")}
                      className="w-full p-3 rounded-xl border border-white/5 bg-white/5 hover:bg-white/[0.08] hover:border-white/15 transition-all text-left flex items-center justify-between"
                    >
                      <div>
                        <span className="block text-xs font-semibold text-white">guest-explorer@gmail.com</span>
                        <span className="block text-[9px] text-gray-400">Guest Explorer Account</span>
                      </div>
                      <span className="text-[8px] bg-white/5 px-2 py-0.5 rounded text-gray-400 font-mono">Simulated</span>
                    </button>

                    <button 
                      onClick={() => setShowCustomGoogleInput(true)}
                      className="w-full py-2 text-center text-[10px] text-brand-violet hover:text-brand-magenta transition-all font-semibold"
                    >
                  ...
                  <div className="space-y-3">
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold text-gray-400 uppercase tracking-wider block">Google Email</label>
                      <input 
                        type="email"
